...
class AccountDeletion {
  ...
  static Future<bool> confirmAndDelete(
    ...
    if (confirmCtrl.text.trim().toUpperCase() != 'DELETE') {
      _snack(
          context, 'Please type DELETE to confirm.', const Color(0xFFE53935));
      return false;
    }
    ...
    try {
      ...
      if (res['success'] == true) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          _snack(context, 'Your account has been deleted.',
              const Color(0xFF43A047));
        });

        return true;
      } else {
        _snack(context, res['message']?.toString() ?? 'Delete failed.',
  ...
  static Future<Map<String, dynamic>> _deleteAccountWithPassword({
    ...
    try {
      return await TellmeAccountApi().deleteAccount(
        email: email,
        password: password,
        confirm: confirm,
        feedback: feedback,
